import Groq from "groq-sdk";
import { env } from "./env";

interface TranscriptSegment {
  time: string;
  text: string;
}

interface Lecture {
  id: string;
  title: string;
  transcript: TranscriptSegment[];
  summary: string[];
}

let groqClient: Groq | null = null;

function getGroqClient(): Groq {
  if (groqClient) return groqClient;

  const apiKey = env.GROQ_API_KEY;
  if (!apiKey) {
    throw new Error("GROQ_API_KEY is not set");
  }
  groqClient = new Groq({ apiKey });
  return groqClient;
}

export async function askGroqAboutLecture(
  lecture: Lecture,
  question: string,
): Promise<{ answer: string; timestampRef?: string }> {
  const groq = getGroqClient();
  const transcriptContext = lecture.transcript
    .map((seg) => `[${seg.time}] ${seg.text}`)
    .join("\n\n");

  const systemPrompt = `Ты — ИИ-аналитик лекции UniOS AI. В переданном транскрипте четко выделяй и анализируй только речи Ведущего/Преподавателя ([Преподаватель / Ведущий]). Игнорируй слова-паразиты, случайные шумы, фоновый гул и технические паузы. Формируй конспект и ответы строго по сути сказанного лектором.

ТВОЯ РОЛЬ:
- Объясняй сложные концепции простым языком, используя аналогии и примеры
- Помогай студентам связывать новые знания с уже известными
- Задавай наводящие вопросы для развития критического мышления
- Структурируй ответы: определение → суть → пример → применение
- Адаптируй сложность под уровень студента (по умолчанию — бакалавриат)

ПРАВИЛА:
1. Анализ строго по речи лектора: секции с пометкой «[Преподаватель / Ведущий]» — это источник контента; всё остальное (шум, паузы, реплики слушателей, слова-паразиты) отбрасывай
2. Основывайся на транскрипции лекции, но можешь дополнять знаниями из предметной области
3. Если вопрос выходит за рамки лекции — честно укажи это, но всё равно дай полезный ответ
4. ОБЯЗАТЕЛЬНО указывай таймкод [MM:SS] из транскрипции, если ответ основан на лекции
5. Тон: поддерживающий, академичный, но доступный

ФОРМАТ ОТВЕТА:
[MM:SS] Краткий прямой ответ (если есть в лекции) или "Вне лекции:" + ответ
💡 Развёрнутое объяснение с примером/аналогией
🔗 Связь с другими темами лекции (если есть)
❓ Наводящий вопрос для самопроверки

Транскрипция лекции "${lecture.title}":
${transcriptContext}

Краткий конспект:
${lecture.summary.join("\n")}`;

  try {
    const completion = await groq.chat.completions.create({
      model: "openai/gpt-oss-20b",
      messages: [
        { role: "system", content: systemPrompt },
        { role: "user", content: question },
      ],
      temperature: 0.3,
      max_tokens: 1024,
    });

    const answer = completion.choices[0]?.message?.content ?? "Не удалось получить ответ от AI.";

    const timestampMatch = answer.match(/\[(\d{2}:\d{2})\]/);
    const timestampRef = timestampMatch ? timestampMatch[1] : undefined;

    return { answer, timestampRef };
  } catch (error) {
    console.error("Groq API error:", error);
    const message = error instanceof Error ? error.message : "Неизвестная ошибка";
    if (message.includes("API key") || message.includes("authentication")) {
      throw new Error("Неверный API ключ Groq. Проверьте GROQ_API_KEY в переменных окружения.");
    }
    throw error;
  }
}